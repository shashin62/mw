<nav class="navbar navbar-inverse navbar-fixed-top cnav" role="navigation" style="background-color: #fff;">
      <div class="container" style="margin-bottom: auto !important">
        <div class="navbar-header" style="padding-bottom: 10px;">
          <button type="button" class="navbar-toggle collapsed btn-h1 btn-m" data-toggle="collapse" data-target="#navbar" aria-expanded="false" aria-controls="navbar" style=" border: 0 !important;border-radius: 0;margin-top: 14px;">
            <span class="fa fa-navicon btn-cl btn-cls" style="font-size: 20px;"></span>
          </button>
          <a class="navbar-brand" href="http://<?php echo $_SERVER['HTTP_HOST'] ?>/index.php">
          <span id="logo" style="top: 50%;">
          <img alt="" src="img/mlogo-011.png" class="visible-lg visible-md visible-sm"/>
          <img alt="" src="img/mlogo-011.png" height="" width="60px" class="visible-xs"/>
          </span> 
          <span id="l-text"><img alt="" src="img/ltext-01.png" width="130px"/></span>
          </a>
        </div>
        <div id="navbar" class="navbar-collapse collapse " style="margin-top: 6px;">
          <ul class="nav navbar-nav navbar-right" id="nav-menu-ul">
	      	<li>
	       		<a href="http://<?php echo $_SERVER['HTTP_HOST'] ?>//about-us.php" class="header1a active-about">
	        		About
	       		</a>
	      	</li>
	      	<li>
	       		<a href="http://<?php echo $_SERVER['HTTP_HOST'] ?>/how-it-works.php" class="header1a active-how-it-work">
	        		How It Works
	       		</a>
	      	</li>
	      	<li class="dropdown">
	       		<a href="http://<?php echo $_SERVER['HTTP_HOST'] ?>/our-work.php">
	        		Our Work</a><a href="#" class="active-our-work" role="button" aria-haspopup="true" aria-expanded="false" title="" tmp_title="" style="width: 20px; position: absolute;
	        		 top: 0; left: 79px;"><span class="caret"></span>
	       		</a>
	       		<ul class="dropdown-menu">
      <li><a href="http://<?php echo $_SERVER['HTTP_HOST'] ?>/products/personalized-gifts.php">Personalized Gifts</a></li>
      <li><a href="http://<?php echo $_SERVER['HTTP_HOST'] ?>/products/corporate-gifts.php">Corporate Gifts</a></li>
      <li><a href="http://<?php echo $_SERVER['HTTP_HOST'] ?>/products/unique-jewellery.php">Unique Jewellery</a></li>
      <li><a href="http://<?php echo $_SERVER['HTTP_HOST'] ?>/products/event-specific-gifts.php">Event-Specific Gifts</a></li>
      <li><a href="http://<?php echo $_SERVER['HTTP_HOST'] ?>/products/occasion-based.php">Occasion-Based</a></li>
    </ul>
	      	</li>
	      	<li>
	       		<a href="http://<?php echo $_SERVER['HTTP_HOST'] ?>/submitYourIdea.php" -class="header1a active-sub-idea">
	        		Submit Your Idea
	       		</a>
	      	</li>
	      	<li>
	       		<a href="http://<?php echo $_SERVER['HTTP_HOST'] ?>/contact-us.php" class="header1a active-contact">
	        		Contact
	       		</a>
	      	</li>
	      </ul>
        </div><!--/.navbar-collapse -->
      </div>
    </nav>