<!doctype html>
<!--[if lt IE 7]>      <html class="no-js lt-ie9 lt-ie8 lt-ie7" lang=""> <![endif]-->
<!--[if IE 7]>         <html class="no-js lt-ie9 lt-ie8" lang=""> <![endif]-->
<!--[if IE 8]>         <html class="no-js lt-ie9" lang=""> <![endif]-->
<!--[if gt IE 8]><!--> <html class="no-js" lang=""> <!--<![endif]-->
    <head>
    <title>3D Printing Service, Buy 3D Printed Customized Gifts, Jewellery, Decors Online - MakeWhale</title>
    <meta name="title" content="3D Printing Service, Buy 3D Printed Customized Gifts, Jewellery, Decors Online - MakeWhale" />
    <meta name="robots" content="index, follow">
    <meta name="description" content="Design & Create 3D Printed personalized corporate gifts, decors, 3D printed jewellery, customised baby gifts & more online with our 3D Printing service in India at MakeWhale" />
    <meta name="keywords" content="3D printing technology, 3D printing in india, 3D printed jewellery, 3D products, 3D decoration, buy 3D printed objects, 3D-printed home decor, top 3d printing company india, best 3d printing company india,  3D printed office accessories," />
	<meta name="p:domain_verify" content="e227b815d41a1729e3823bd23b0da11b"/>
	<META NAME="ROBOTS" CONTENT="NOINDEX, NOFOLLOW">
    	<?php include("includes/include_css1.php") ?>
    	
        <style>

        	.dropdown-menu>li>a {
    display: block;
    padding: 3px 20px;
    clear: both;
    font-weight: 400;
    line-height: 1.42857143;
    color: #333;
    white-space: nowrap; 
}

        	.navbar-toggle{
				margin-bottom: 0px;
			}
			#footer .home{
				color:#F0693C !important;
			}
.navbar-inverse .navbar-nav .open .dropdown-menu>li>a:focus,.navbar-inverse .navbar-nav .open .dropdown-menu>li>a:hover {
        color:#9d9d9d;
        background-color:#f1f1f1
    }

    .dropdown:hover .dropdown-menu, .dropdown:active .dropdown-menu {
    display: block;
}

.dropdown-menu {
    position: absolute;
    top: 100%;
    left: 0;
    z-index: 1000;
    display: none;
    float: left;
    min-width: 160px;
    padding: 5px 0;
    margin: 2px 0 0;
    font-size: 18px;
    text-align: left;
    list-style: none;
    background-color: #fff;
    -webkit-background-clip: padding-box;
    background-clip: padding-box;
    border: 1px solid #ccc;
    border: 1px solid rgba(0,0,0,.15);
    border-radius: 4px;
    -webkit-box-shadow: 0 6px 12px rgba(0,0,0,.175);
    box-shadow: 0 6px 12px rgba(0,0,0,.175);
}
        @media(max-width:769px) {
         .dropdown-menu {
    position: relative;
    top: 100%;
    left: 0;
    z-index: 1000;
    display: none;
    float: left;
    min-width: 160px;
    padding: 5px 0;
    margin: 2px 0 0;
    font-size: 18px;
    text-align: left;
    list-style: none;
    background-color: #fff;
    -webkit-background-clip: padding-box;
    background-clip: padding-box;
    border: 1px solid #ccc;
    border: 1px solid rgba(0,0,0,.15);
    border-radius: 4px;
    -webkit-box-shadow: 0 6px 12px rgba(0,0,0,.175);
    box-shadow: 0 6px 12px rgba(0,0,0,.175);
}
}
      }
			@media(max-width: 640px){
				.small-tx-circle{
					font-size: 23px;
				}
			}
			@media(max-width: 480px){
				.small-tx-circle{
					font-size: 21px;
				}
			}
			@media(max-width: 385px){
				.small-tx-circle{
					font-size: 20px;
				}
				.stc{
					width: 80%;
/*					margin: 0 auto;*/
				}
			}
        </style>
        
    <!-- Google Analytics: change UA-XXXXX-X to be your site's ID. -->
    
  <?php include("includes/google_analytics.php") ?>
        
    </head>
    <body>
    
    <?php include("includes/browser_upgrade.php") ?>
    
    <!--############ Loader Page ###################-->

<div class="loader" id="">
	<div class="inner-loader">
		<img src="img/MakeWhale Logo-01.png" id="3d-transform" alt="MakeWhale"/><br><br>
		<img src="img/MakeWhale Logo-02.png" alt="MakeWhale">
	</div>
</div>

<!--############End Loader Page ###################-->
    
    <?php include('includes/header2.php'); ?>
    
     <!-- Header Carousel -->
    <header id="myCarousel" class="carousel slide">
        <!-- Indicators -->
        <ol class="carousel-indicators">
            <li data-target="#myCarousel" data-slide-to="0" class="active"></li>
            <li data-target="#myCarousel" data-slide-to="1"></li>
            <li data-target="#myCarousel" data-slide-to="2"></li>
			<li data-target="#myCarousel" data-slide-to="3"></li>
        </ol>

        <!-- Wrapper for slides -->
        <div class="carousel-inner">
            <div class="item active">
                <div class="fill home1" title="Unique 3D printed ideas">
                	
                	<div class="carousel-caption animated fadeIn">
                	<h1 class="slider-cap">
                	<span>Rediscover</span><br>
                   Gifting
                	</h1>
                	</div>
                	
                </div>
                
            </div>
            <div class="item">
                <div class="fill home2">
                
                	<div class="carousel-caption animated fadeIn">
                    <h1 class="slider-cap">
                	<span>For</span><br>
                  Every Occasion
                	</h1>
                	</div>
                
                </div>
            </div>
            <div class="item">
                <div class="fill home3">
                	
                	<div class="carousel-caption animated fadeIn">
                   <h1 class="slider-cap">
                   <span>Tailored</span><br>
                  For You
                	</h1>
                	</div>
                	
                </div>
            </div>
			<div class="item">
                <div class="fill home4">
                	
                	<div class="carousel-caption animated fadeIn">
                   <h1 class="slider-cap">
                	<span>Designed</span><br>
                	From Scratch
                	</h1>
                	</div>
                	
                </div>
            </div>
        </div>

        <!-- Controls -->
        <!--<a class="left carousel-control" href="#myCarousel" data-slide="prev">
            <span class="icon-prev"></span>
        </a>
        <a class="right carousel-control" href="#myCarousel" data-slide="next">
            <span class="icon-next"></span>
        </a>-->
    </header>
    
    <!--Divider for Premium-->
    
    <div class="container-fluid bar text-center" style="margin-bottom: 120px;">
    	<h3 class="p3p" style="color: #FFFFFF">PREMIUM 3D PRINTED GIFTS</h3>
    </div>

    <!-- Main jumbotron for a primary marketing message or call to action -->

    <div class="container" style="">
      <!-- Example row of columns -->
      <div class="row" style="">
        <div class="col-md-6" style="padding-right: 50px;padding-left: 50px;margin-bottom: 60px;">
          <div class="text-center">
          	<a href="our-work.php"><button class="g-img" style="margin-left: 30px;"></button></a>
          </div>
          <h2 class="text-center text-muted visible-lg visible-md visible-sm" style="border-bottom: 1px solid #30BAA4;padding-bottom: 20px; font-family: 'lubalgraph_bk_btbook';margin-left: 80px;margin-right: 80px;">
          our work
          </h2>
          <h2 class="text-center text-muted visible-xs small-tx-circle" style="border-bottom: 1px solid #30BAA4;padding-bottom: 20px; font-family: 'lubalgraph_bk_btbook';font-size: 25px;margin-left: 30px;margin-right: 30px;">
          our work
          </h2>
          <p class="text-center" style="margin-top: 20px;font-family: 'questrialregular';">A collection of products designed by us to give you a sense of the infinite possibilities we offer.  </p>
          <p class="text-center" style="margin-top: 40px;">
          <a style="font-family: 'lubalgraph_bk_btbook';letter-spacing: 2px;" class="btn btn-danger" href="our-work.php" role="button">VIEW</a></p>
        </div>
        <div class="col-md-6" style="padding-left: 50px;padding-right: 50px;">
          <div class="text-center" id="s-cart">
          	<a href="how-it-works.php"><button class="g-img1"></button></a>
          </div>
           <h2 class="text-center text-muted visible-lg visible-md visible-sm" style="border-bottom: 1px solid #30BAA4;padding-bottom: 20px; font-family: 'lubalgraph_bk_btbook';margin-left: 80px;margin-right: 80px;">
          how it works
          </h2>
          <h2 class="text-center text-muted visible-xs small-tx-circle stc" style="border-bottom: 1px solid #30BAA4;padding-bottom: 20px; font-family: 'lubalgraph_bk_btbook';font-size: 25px;margin-left: 30px;margin-right: 30px;">
         	how it works
          </h2>
          <p class="text-center" style="margin-top: 20px;font-family: 'questrialregular';">A quick guide to how we work and how you can order a personalized 3D printed product. </p>
          <p class="text-center" style="margin-top: 40px;">
          <a style="font-family: 'lubalgraph_bk_btbook';letter-spacing: 2px;" class="btn btn-danger" href="how-it-works.php" role="button">VIEW</a>
          </p>
       </div>
      </div>
	  
	  <div class="row">.
	  <div>
     <div class="row" style="margin-top:10px;">
		
        <div class="col-md-12" style="padding:0px;">
		<h4 class=" text-center banner-overlay-heading" style="color: #000 ! important; background-color: #ededed; width: 100%; margin-bottom: 0px !important; padding: 10px 0 10px 0;">MEDIA</h4>
          <p class="text-center" style="background-color:#ededed; padding:10px 0px; margin-bottom: 0px !important; border-top: 1px solid #cacaca;">
            <a href="http://<?php echo $_SERVER['HTTP_HOST'] ?>/test1/test.php" target="_blank">&nbsp;&nbsp;&nbsp;&nbsp;<img src="img/3D_printing_industry.png" alt="" style="padding-right: 30px;" /></a>
            <a href="http://<?php echo $_SERVER['HTTP_HOST'] ?>/test1/test.php" target="_blank">
            <img src="img/ADLogo.png" alt="" style="padding-right: 30px;" />
            </a>
            <a href="http://<?php echo $_SERVER['HTTP_HOST'] ?>/test1/test.php" target="_blank">
            <img src="img/Brand_BrownPaperBag_289.png" alt="" style="padding-right: 30px;" />
            </a>
            <a href="http://<?php echo $_SERVER['HTTP_HOST'] ?>/test1/test.php" target="_blank">
           <img src="img/Mint.png" alt="" style="padding-right: 30px;" />
            </a>
			 <a href="http://<?php echo $_SERVER['HTTP_HOST'] ?>/test1/test.php" target="_blank">
           <img src="img/vogue.png" alt="" style="padding-right: 30px;" />
            </a>
            </p>
        </div>
      </div>
    </div> <!-- /container --> 
    

    <?php include('includes/footer11.php');?>
    
    <?php include("includes/include_js.php") ?>

	<div id="footer" class="container-fluid" style="background-color: #ededed; color: #777;">
	<br>
		 <p style="font-size: 11px">MakeWhale is India’s first 3D printing and design company that's focused on creating beautiful 3D printed personalized products. Our aim is to offer you and your organization a way to give unique personalized gifts that enhance relationships and occasions.MakeWhale has made 3D Printing in India available to the consumers and has given a modern twist to the traditional ways of corporate gifting, festive gifting and more.</p>
		 <p style="font-size: 11px;">One can now change the way they give diwali gifts, wedding gifts and corporate gifts by replacing the mainstream products with 3D Printed customized gifts. It's a simple procedure, where one can just come up with cool gift ideas and have it designed and printed at the same place. This makes corporate gifting much easier as the products can completely incorporate the brand language and be the ultimate customized corporate gift. MakeWhale takes customization to a whole new level with fingerprint cufflinks and fingerprint pendants, customized jewellery, personalized accessories for men and women, unique wedding gifts, custom-made stationery, unique diwali gifts, festive products, premium corporate gifts and so much more! 3D printing in India has never been so easily accessible before.</p>
	<br>
	</div>
	
	
    </body>
</html>
